...
IN_FILE = PROJECT_ROOT / "data" / "processed" / "eeg_features_sub01_sub05_session_norm.csv"
OUT_FILE = PROJECT_ROOT / "data" / "processed" / "eeg_baseline_session_norm_loso_results.csv"

df = pd.read_csv(IN_FILE)

print("Loaded EEG session-normalized data:", df.shape)
...
# Use only session-normalized EEG features
feature_cols = [
    col for col in df.columns
    if col.startswith("z_log_")
]
...
print(f"\nSaved session-normalized EEG LOSO results to: {OUT_FILE}")


# Logistic Regression coefficients trained on all subjects
print("\n" + "#" * 80)
